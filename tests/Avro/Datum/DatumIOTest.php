...
class DatumIOTest extends TestCase
{
    ...
    public function testDatumRoundTrip($schemaJson, $datum, $binary)
    {
        ...
        self::assertEquals($binary, $output, sprintf(
            "expected: %s\n  actual: %s",
        ...
        self::assertEquals($datum, $readDatum);
    }
    ...
    public function defaultProvider()
    {
        return [
            ...
            ['{"name":"x","type":"record","fields":[{"name":"label","type":"int"}]}', '{"label":7}', ['label' => 7]],
        ];
    }
    ...
    public function testFieldDefaultValue($fieldSchemaJson, $defaultJson, $defaultValue)
    {
        ...
        $reader = new AvroIODatumReader($writersSchema, $readersSchema);
        $record = $reader->read(new AvroIOBinaryDecoder(new AvroStringIO()));
        if (array_key_exists('f', $record)) {
            self::assertEquals($defaultValue, $record['f']);
        } else {
            self::assertTrue(false, sprintf('expected field record[f]: %s', print_r($record, true)));
        }
    }
}
