...
class FileIOTest extends TestCase
{
    ...
    private function read()
    {
        ...
        $reader   = AvroDataIO::openFile($fileName);
    ...
    public function testReading()
    {
        ...
        self::assertEquals($expected, $this->read());
    }

    /**
     * Doesn't work because due to Avro format spf° mean that no two encodings of the same Data will be binary equal.
     */
    public function disabledTestRoundTrip()
    {
        ...
        $writer     = AvroDataIO::openFile($outFile, 'w', $schema);
        ...
        if ($oldData !== $newData) {
            ...
            self::fail("Round trip failed, files not equal:\n$diff");
        }
        self::assertTrue(true, 'Dummy assert to prevent this test from being marked as risky');
    }
}
